 ...
 * de ejecución.
 * 
 * Las sentencias pueden presentarse en un primer nivel si no se encuentras dentro de ningún
 * bloque, o dentro de otras sentencias en forma de bloque. 
 */
...
class typeofNode :
 ...
 public:
   ...
   /**
    * Método que ejecuta nodo. Imprime en la salida estándar el 
 ...
 * 
 * Dados un bloque de sentencias este nodo permite romper la secuencia
 ...
 * secuencia de sentencias en un bloque de estas, siguiendo en la siguiente sentencia
 * del bloque. Adicionalmente se puede especificar el número de bloques anidados 
 * de los que se desea salir. 
 * 
...
class labelNode :
 ...
 public:
   ...
   * \param stmt Nodo sentencia que será ejecutado la llamarse 
   */
 ...
 * 
 * Esta excepción se produce cuando se ejecuta una sentencia goto. Sirve para que 
 * no se siga la ejecución en la siguiente sentencia.
 */
class gotoException :
   public std::exception {
 public:
...
class gotoNode :
 ...
 public:
   /**
    * Constructor de clase
    * \param node Nodo que se corresponde con el identificador de la 
    * etiqueta. 
    */
   gotoNode (runNode* node);
   /**
    * Método que ejecuta el nodo. Accede al nodo sentencia
    * referenciado por el identificador de etiqueta pasado en la inicialización y lo ejecuta.
 ...
 * de forma que las tablas de símbolos a las que se accederá será a la del objeto, y si 
 * el recurso solitado no se encuentra se hará sobre el contexto en 
 * uso.
 * 
 * Se mantiene una pila de contextos principles al que se le intruduce 
...
class withNode : public runNode {
   public: 
      ...
      /**
       * Método que ejecuta el nodo. Establece el contenxto 
       * del objeto como contexto principal. 
       */
      void run();
      /**
       * Añade un nodo como objeto de contexto 
       * principal en la pila de objetos.
...
class datInfoNode : public runNode {
   public:
      ...
      /**
       * Ejecución del nodo. Imprime en la salida entándar
       * el tipo del nodo, identificador asociado si tuviera, número 
       * de elementos en caso de ser compuesto, el valor  y el número 
       * de referencias
       */
      void run ();
   private:
 ...
 * sentencia try...catch (\see tryNode), produciendose la ejecución de las 
 * sentencias correspondiente al bloque catch.
 * 
 * La exepción guarda un valor que podra ser usado en 
 * el bloque catch.
 */
class throwException :
   public std::exception {
 ...
 * 
 * Este nodo permite lanzar una excepción para que sea capturada en un bloque
 * try...catch (\see tryNode). La excepción puede tener un valor que será pasado 
 * al bloque catch.
 * 
 */ 
class throwNode : public runNode {
   public:
      ...
      /**
       * Método que ejecuta el nodo. Lanza la excepción para que sea capturada
       * por un bloque try..catch. Si la excepción no es capturada se produce un 
       * error.
       */
      void run ();
   private:
 ...
 * si se lanza una excepción throw esta será capturada 
 * y pasará a ejecutarse el bloque catch. Además el bloque catch es
 * precidido de una expresión en el que se especifica un id al que será 
 * asignado el valor de la excepción.
 */
class tryNode : public runNode {
   public:
       ...
       * \param id Nodo que hace de id al que se le asignará el valro de la excepción

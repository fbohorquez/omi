 ...
 * 
 * Las tablas de símbolos pueden contener nodos que guarden cierta relación. Por
 * ejemplo se puede utilizar una tabla para las variables, otras paras las funciones...
 * 
 * \see refNode
 */
class symbolsTable {
   public:
       ...
       * \return Obtiene un iterador de la tabla de símbolos que apunta a la primera 
       * referencia
       */
      isymTable begin ();
      /**
       * Obtiene un iterador de tabla de símbolos al final de la tabla
       * \return Obtiene un iterador de la tabla de símbolos que apunta al final de la tabla
       */
      isymTable end ();
      /**
       * Obtiene un iterador de tabla de símbolos a la primera referencia
      ...
      isymTable end () const;
      /**
       * Busca y devuelve un iterador posicionado en el elemento de la tabla de 
       * símbolos que apunta al nodo dado, según la función de comparación de 
       * nodos. 
       * \param node Nodo a buscar
       * \return Iterador a la referencia del elemento buscado. Al final de la tabla si no se encuentra.
       */
      isymTable find (runNode* node); 
      /**
       * Busca y devuelve la referencia de la tabla que apunta a un nodo dado, según 
       * la función de comparación de nodos. Si no es enecontrado se crea una referencia 
       * en la tabla cuyo valor es el nodo por defecto. Si es encontrado el nodo por defecto es liberado.
       * \param node Nodo a buscar
      ...
      void clearRef ();
      /**
       * Determina si la tabla de símbolos está vacia.
      ...
      int size () const;
      /**
       * Sobrecarga el operador de acceso. Permite acceder 
       * a una referencia de la tabla de símbolos
       * a partir de una clave. Si la clave 
       * no es encontrada devuelve un nodo nulo
       *
       * \param key Clave correspondiente a la referencia a buscar
       * \return Nodo al que apunta la referencia dada por la clave. Si 
       * no existe una referencia se devuelve el nodo nulo
       */
     ...
     /**
       * Sobrecarga el operador de acceso. Permite acceder 
       * a una referencia de la tabla de símbolos
       * a partir otra referencia, comparándose las claves. Si  
       * no es encontrada devuelve un nodo nulo
       *
       * \param key Clave correspodniente a la referencia a buscar
       * \return Nodo al que apunta la referencia dada por la clave. Si 
       * no se encuentra una referencia se devuelve el nodo nulo
       */
      runNode* &operator[] (refNode* key);
      /**
       * Sobrecarga el operador de acceso. Permite acceder 
       * a una referencia de la tabla de símbolos
       * a partir un nodo que hará de referencia, comparándose las 
       * claves. Si  no es encontrada devuelve un nodo nulo
       *
       ...
       * Sobrecarga el operador de acceso. Permite acceder 
       * a una referencia de la tabla de símbolos
       * a partir una posición. Si  no es encontrada devuelve un nodo nulo
       *
      ...
      /**
       * Obtiene la referencia de la tabla de símbolos en la posición dada. Si
       * está fuera de rango se devuelve NULL
       * \param val Posición
       ...
       * \return Última referencia de la tábla de símbolos.
       */
      refNode* last ();
      /**
      ...
      /**
       * Inserta una referencia en una posición dado.
 ...
 * y se compone de dos referencias una a la referencia corresdiente en la tabla 
 * de símbolos y otra a su valor. 
 * 
...
class isymTable {
   public:
      ...
      isymTable ();
      /**
       * Constructor de la clase. Crea un iterador en una determionada posición
      ...
      bool operator!=(const isymTable &elem) const;
      /**
       * Sobrecarga del operador de incremento y asignación. Avanza el iterador
       * una posición en la tabla de símbolos.
       * \return Devuelve el iterador en la nueva posición.
       */
      isymTable& operator++();
      /**
       * Sobrecarga del operador de decremento y asignación. Retrocede el iterador
       * una posición en la tabla de símbolos.
       * \return Devuelve el iterador en la nueva posición.
       */
      isymTable& operator--();
   /**
       * Sobrecarga del operador de asignación e incremento. Avanza el iterador
       * una posición en la tabla de símbolos.
       * \return Devuelve el iterador en la nueva posición.
       */
      isymTable operator++(int);
       /**
       * Sobrecarga del operador de asignación e decremento. Retrocede el iterador
       * una posición en la tabla de símbolos.
       * \return Devuelve el iterador en la nueva posición.
       */
      isymTable operator--(int);
       /**
       * Obtiene la posición numñérica del iterador

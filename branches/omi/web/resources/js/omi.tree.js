function tree (obj) {
   ...
   this.node_canvas.height = (tree.row + 4) * CONFIG.NODE('ROW_SIZE');
   this.node_canvas.width = (node.last_col + 1) * CONFIG.NODE('COL_SIZE') + 50;
   this.info_canvas = document.getElementById(CONFIG.TREE('INFO_ID'));
   this.info_canvas.height = (tree.row +4) * CONFIG.NODE('ROW_SIZE');
   this.info_canvas.width = (node.last_col + 1) * CONFIG.NODE('COL_SIZE') + 50;
...
tree.row = 0;
tree.current_stmt = null;
tree.before_stmt = null;
tree.bbefore_stmt = null;
tree.play = null;
...
tree.run_next_stmt = function () {
   ...
   //~ before_stmt = current_stmt;
      ...
      if (step = tree.omi.run_list.next ()) {
         tree.bbefore_stmt = tree.before_stmt;
         tree.before_stmt = tree.current_stmt;
         tree.current_stmt = idMap[step.id];
         run.action(step, step.action, step.attrs);
      }
      ...
      if (tree.omi.run_list.list.length == 0 ){
         if(!run.loop && tree.omi.run_list.last.action != "inputNode" && tree.omi.run_list.last.action != "input"){
            console_omi.omi.setValues("<span style=\"color:#1198D6;\">La ejecución del programa ha finalizado.</span>");
            console_omi.omi.draw(true);
            endStep();
         }else{
      ...
      if (tree.current_stmt && tree.bbefore_stmt && tree.current_stmt.id == tree.bbefore_stmt.id){
         while (tree.omi.run_list.list.length && tree.is_next_stmt()){
            if (step = tree.omi.run_list.next ()) {
               run.action(step, step.action, step.attrs);
            }
      ...
      console.log (tree.current_stmt && tree.current_stmt.id,  tree.before_stmt && tree.before_stmt.id, tree.bbefore_stmt && tree.bbefore_stmt.id);
};

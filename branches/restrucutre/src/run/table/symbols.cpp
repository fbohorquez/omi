...
symbolsTable::symbolsTable (const symbolsTable* table) : table_(table->table_) {}
...
refNode* &symbolsTable::push_back (refNode* node) {
   table_.push_back (node);
   return table_.back();
} 

void symbolsTable::pop_back () {
   if (table_.size() > 0)
      table_.pop_back ();
}
...
runNode* &symbolsTable::operator[] (const string key) {
...
bool symbolsTable::empty () {
   return table_.empty();
}

int symbolsTable::size () const {
   return table_.size();
}

refNode* symbolsTable::last () {
   return table_.back();
}

refNode* symbolsTable::first () {
   if (table_.size())
      return table_.front();
   else
      return NULL;
...
int symbolsTable::findPosition (refNode* node) {
   for (int i = 0; i < table_.size(); ++i){
...
//-------------------------------------------------------------------------------------------
//isymTable
isymTable::isymTable () : sym_ (NULL), pos_ (-1), first(NULL), second(NULL) {}

isymTable::isymTable (const symbolsTable* sym , unsigned int pos): 
   sym_ (new symbolsTable(sym)), 
   pos_ (pos), 
   first(NULL), 
   second(NULL) {
   setPair();
...
bool isymTable::operator==(const isymTable &elem) const {
...
isymTable isymTable::operator++(int) {
   this->pos_ ++;
   setPair();
   return *this;
}
...
unsigned int isymTable::getPos () { 
   return pos_;
}


void isymTable::setPair () {

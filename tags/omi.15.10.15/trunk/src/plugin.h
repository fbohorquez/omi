...
class PluginsLoader {
   ...
   static PluginsLoader* singleton;
   PluginsLoader ();
   vector<PluginLoader*> libs;
 public:
   static PluginsLoader* getPluginsLoader () { 
      if (!singleton)
         singleton = new PluginsLoader();
      return singleton;
   }
   void add (string key,runNode*(*val)(listNode*)) {
      creates[key] = val;
   }
   void addPlugin (PluginLoader* plugin) {
      libs.push_back (plugin);
   }
   void print () {
      map<string, runNode*(*)(listNode*)>::iterator ini = creates.begin();
      for (; ini != creates.end(); ini++){
         cerr << ini->first << endl;
      }
   }
...
class extensionNode {
   public:
      extensionNode (listNode* list = NULL) : list_(list) {}
      vector<runNode*> getParams (bool require = true) {
         if (!list_ && require)
            throw errorException("Wrong number parameters", "extensionNode: some param is require", ERROR_PARAMS);
         list_->run();
         return list_->listvalue ();
      }
   private:
      listNode* list_;
};

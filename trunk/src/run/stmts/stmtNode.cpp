...
//-------------------------------------------------------------------------------------------
//labelNode
labelNode::labelNode (runNode* id, runNode* stmt): 
   id_ (id), stmt_(stmt) {}
...
void gotoNode::run () {
   ...
   if (idNode* id = dynamic_cast<idNode*>(node1_)){
      if ((id->name()).find("~label:") == string::npos)
         id->setName ("~label:" + id->name());
   }
...
void withNode::push_with (runNode* node) {
   stack_with.push_back (node);
   top_with ++;
}

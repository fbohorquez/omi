 ...
 * \brief Nodo clase.
 *
 * Un nodo clase representa la definición de una clase. Una clase es una
 * estructura funcional utilizada en la programación orientada a objetos.
 *
 * Un nodo clase sirve como interfaz para definir y manipular las clases
 * en ejecución.
 *
 * Todo nodo clase tiene un contexto de tablas de símbolos donde se 
 * referencia a los atributos y métodos de la misma.
 *
 * Para inicializar un nodo clase se precisa del identificador de la misma y un
 * cuerpo.
 *
 * El cuerpo de la clase consiste en una serie de sentencias, que por limitaciones
 ...
 * cambia el contexto activo al interno y se lleva a cabo la ejecución del cuerpo.
 *
 * Los nodos funciones declarados en el cuerpo de la clase serán referenciados
 * desde la tabla de funciones de la clase y son considerados los métodos de la misma.
 * De igual forma las variables son referenciadas mediante la tabla de variables y 
 * son consideradas como los atributos.
 *
 * Una determinada clase puede extender a otra, lo que significa que heredará sus
 * métodos y atributos.
 *
 * Los nodos clase presentan un mecanismo para crear nuevas instancias de los
 * mismos denominadas objetos. Este mecanismo es utilizado por nodos como el
 ...
 * El objeto se puede asignar a un nodo id como si fuera un tipo de dato, 
 * guardándose asi una referencia en la tabla de variables activa.
 * 
 * Las clases disponen de un contexto interno con tablas para funciones y variables 
...
class classNode :
 ...
 public:
   ...
   /**
    * Constructor que inicializa un nodo clase a partir de un nodo identificador
    *  y un nodo cuerpo.
    * \param id Identificador de la clase.
    * \param body Cuerpo de la clase.
    * \param extends Clase de la que extiende si es el caso.
    */
   classNode (runNode* id, runNode* body, runNode *extends = NULL);
   /**
    * Lleva a cabo la ejecución del nodo clase correspondiente a la declaración
    * de la misma. Para ello crea una referencia en la tabla de clases activa
    * a si misma, genera el contexto interno, y luego, tras cambiar el contexto en uso, ejecuta el cuerpo.
    */
   void run ();
   /**
    * Crea y devuelve una instancia de la clase. Para ello copia la tabla de 
    * variables del contexto de  la clase  en un nodo objeto, y busca una función
    * en la tabla de funciones correspondiente al nombre de la clase. 
    * Esta función, considerada constructor, será ejecutada con los parámetros indicados.
    */
   ...
   /**
    * Obtiene el identificador de la clase como una cadena de caracteres.
    * \return ID de la clase como cadena de caracteres.
    */
   inline string name(){
    ...
    * \return Método correspondiente a la clave.
    */
   runNode * getMethod (runNode * key);
   /**
    * A partir de una clave obtiene la referencia al método cuyo identificador
    * se corresponde con la misma. Si no existe se devuelve NULL.
    * \param key Nombre del método.
    * \return Referencia al método correspondietne a la clave.
    */
 ...
 * del contexto de la clase que se llame igual que esta, este método es 
 * llamado como método constructor. 
 */
class newNode :
   public nexpNode {
 public:
   ...
   /**
    * Lleva a cabo la ejecución del nodo. Para ello busca en la tabla de clases
    * activa el nodo correspondiente al identificador utilizado en la inicialización. 
    * Crea un objeto a partir de la tabla de símbolos de variables de la clase. 
    * Luego accede y ejecuta el método constructor  de la clase, pasándole 
    * los parámetros con el que fue inicializado. 
    * Por último toma como valor la instancia creada.
    */
   void run ();
 private:
   runNode *id_, *params_;
 ...
 *
 * Este nodo es del tipo expresión no definida. Toma como valor la clase 
 * padre del objeto que opera sobre la tabla de símbolos actual.
 *
 */
class parentNode : public thisNode {
   public:
      ...
      parentNode ();
      /**
       * Obtiene el método correspondiente a la clave dada de la clase 
       * que opera como padre en el contexto actual.
 ...
 * \brief Nodo estático.
 * 
 * Cualquier nodo función o variable que sea definido durante la ejecución
 * de este nodo tendrá el atributo de estático por lo que pertenecerá a la propia clase. 
 */
 ...
 * 
 * Cualquier nodo función o variable que sea definido durante la ejecución
 * de este nodo tendrá el atributo de privado por lo que solo puede ser 
 * accedido si se produce sin que exista un cambio de contexto. 
 */
...
/**
 * \class namespaceNode
 * \brief Nodo de espacio de nombre.
 * 
 * Este nodo se corresponde con el operador de espacio de nombre. Se
 * utiliza para acceder a atributos y métodos de la clase o la clase padre.
 */
class namespaceNode : public nexpNode {
   public:
      /**
       * Constructor de la clase.
       * \param space Identificador usará para acceder al contexto. 
       * Puede ser un nodo identificador, un nodo estático o un nodo padre.
       * \param key Clave utilizada para obtener el elemento.
       */
      namespaceNode (runNode *space, runNode * key);
      /**
       * Según el nodo dado como espacio se accede al contexto de una
